...
7. Setup 'Backup Emacs Session Service'
7.1 Create ~/.config/systemd/user/backup-emacs-session.service

[Unit]
Description=Backup Doom Emacs Session on file change

[Service]
Type=oneshot
# FIX: Natively create the target directory tree if it does not exist
ExecStartPre=/usr/bin/mkdir -p %h/Org/

# The main copy execution (Using %h to cleanly evaluate /home/username)
ExecStart=/usr/bin/cp %h/.config/emacs/.local/etc/workspaces/.doom-session %h/Org/.doom-session

7.2 Create ~/.config/systemd/user/backup-emacs-session.path

[Unit]
Description=Monitor Doom Emacs session file changes

[Path]
PathChanged=%h/.config/emacs/.local/etc/workspaces/.doom-session

[Install]
WantedBy=default.target

7.3 Run without sudo

systemctl --user daemon-reload
systemctl --user enable --now backup-emacs-session.path
