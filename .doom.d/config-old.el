...
(defun my/generate-dashboard ()
  ...
  (insert
   (propertize
    (or my/dashboard-cache
        (setq my/dashboard-cache (my/dashboard-render-banner)))
...
(defun my/formatter-exists-p (cmd)
  "Return non-nil if CMD exists on the system."
  (executable-find (symbol-name cmd)))

(defun my/select-web-formatter ()
  (cond ((my/formatter-exists-p 'biome) 'biome)
        ((my/formatter-exists-p 'prettier) 'prettier)
        ((my/formatter-exists-p 'eslint) 'eslint)
        (t 'prettier)))
...
;; (after! yaml-mode
;;   (add-hook! yaml-mode (apheleia-mode 0)))
...
;; Lazy load these packages only when they are needed
...
(remove-hook 'web-mode-hook #'rainbow-mode)
(remove-hook 'css-mode-hook #'rainbow-mode)
(remove-hook 'emacs-lisp-mode-hook #'rainbow-mode)

(after! web-mode
  ;; Disable color preview for CSS literals in HTML
...
(after! company
  ...
  ;; (add-hook! prog-mode #'my/enable-path-completion)
...
;; Run with 60sec delay after Emacs startup
(run-with-idle-timer
 60 nil
 (lambda ()
   (require 'appt)
   ...
   (setq appt-disp-window-function
         (lambda (remaining _new-time msg)
           (let ((cleaned-msg (my/appt-clean-message msg)))
             ;; Debug log
             (message ">> APPT RAW MSG:  %s" msg)
             (message ">> APPT CLEANED: %s" cleaned-msg)
   ...
   ;; Refresh agenda → appt every 10 minutes
   (run-at-time "1 min" 600 #'org-agenda-to-appt)))
